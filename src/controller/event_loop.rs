            ...
            TermEvent::Key(key_event) => {
                ...
                if current_overlay != UIOverlay::None {
                                    ...
                                    if !app.rich_search_results.is_empty() {
                                        if let Some(selected_idx) = app.ui.selected {
                                                ...
                                                app.rich_search_results.get(selected_idx)
                                            {
                                                ...
                                                let base_dir = app.fs.active_pane().cwd.clone();
                                                if let Some((file_path, _line_num)) =
                                                    RawSearchResult::parse_file_info_with_base(
                                                        selected_line,
                                                        &base_dir,
                        ...
                        UIOverlay::CommandPalette => {
                            match key_event.code {
                                ...
                                KeyCode::Down => {
                                    let mut app = self.app.lock().await;
                                    app.ui.command_palette.selected =
                                        app.ui.command_palette.selected.saturating_add(1).min(
                                            app.ui.command_palette.filtered.len().saturating_sub(1),
                                        );
                                    Action::NoOp
                                }
                                KeyCode::Enter => {
                                        ...
                                        .get(app.ui.command_palette.selected)
                                    {
                                        match cmd.action.clone() {
                                            crate::model::command_palette::CommandAction::OpenConfig => {
                                                // TODO: Implement opening config
                                                Action::NoOp
                                            },
                                            crate::model::command_palette::CommandAction::Reload => {
                                                Action::ReloadDirectory
                                            },
                                            crate::model::command_palette::CommandAction::NewFile => {
                                                Action::CreateFile
                                            },
                                            crate::model::command_palette::CommandAction::NewFolder => {
                                                Action::CreateDirectory
                                            },
                                            crate::model::command_palette::CommandAction::SearchContent => {
                                                Action::ToggleContentSearch
                                            },
                                            crate::model::command_palette::CommandAction::Custom(_s) => {
                                ...
                                _ => Action::NoOp, // Ignore other keys when command palette is active
                            }
                        }
                    ...
                    match (key_event.code, key_event.modifiers) {
                        ...
                        (KeyCode::Char(':'), _) => Action::ToggleContentSearch,
                        (KeyCode::Char('p'), KeyModifiers::CONTROL) => Action::ToggleCommandPalette,
        ...
        match action {
            ...
            Action::DirectoryScanUpdate { path, update } => {
                ...
                if app.fs.active_pane().cwd == path {
                    match update {
                        crate::fs::dir_scanner::ScanUpdate::Entry(entry) => {
                            app.fs.active_pane_mut().add_incremental_entry(entry);
                            app.redraw = true;
                        }
                        crate::fs::dir_scanner::ScanUpdate::Completed(_count) => {
                        ...
                        crate::fs::dir_scanner::ScanUpdate::Error(e) => {
